 ...
 * 2. The runtime metadata-keys allowlist (`METADATA_KEYS`) equals
 *    `V26_FROZEN_METADATA_KEYS ∪ V27_ADDITIVE_METADATA_KEYS` (so a v2.7
 *    additive key cannot be declared on the contract module without
 *    also being routed through the operation-log writer / replay reader).
...
describe('V27-00 contract invariants', () => {
  ...
  it('keeps METADATA_KEYS == V26_FROZEN ∪ V27_ADDITIVE (same set, same order)', () => {
    ...
    expect(v26.size + v27.size).toBe(METADATA_KEYS.length);
  });
  ...
  it('declares a contract generation marker', () => {
    expect(V27_CONTRACT_GENERATION).toBe(1);
  });
