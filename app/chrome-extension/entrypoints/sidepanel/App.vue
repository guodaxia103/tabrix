    ...
    <div v-show="activeTab === 'element-markers'" class="element-markers-content">
      <div class="px-4 py-4">
        ...
        <div class="em-toolbar">
          <div class="em-search-wrapper">
            ...
            <input
              ...
              :placeholder="getMessage('sidepanelMarkerSearchPlaceholder')"
              type="text"
            />
            <button
              v-if="markerSearch"
          ...
          <button
            ...
            :title="getMessage('sidepanelMarkerAddTitle')"
          >
          ...
          <div class="em-modal">
            <div class="em-modal-header">
              <h3 class="em-modal-title">
                {{
                  editingMarkerId
                    ? getMessage('sidepanelMarkerEditTitle')
                    : getMessage('sidepanelMarkerAddTitle')
                }}
              </h3>
            ...
            <form @submit.prevent="saveMarker" class="em-form">
              <div class="em-form-row">
                <div class="em-field">
                  <label class="em-field-label">{{ getMessage('sidepanelMarkerNameLabel') }}</label>
                  <input
                    v-model="markerForm.name"
                    class="em-input"
                    :placeholder="getMessage('markerNamePlaceholder')"
                    required
                  />
                </div>
              </div>
              ...
              <div class="em-form-row em-form-row-multi">
                <div class="em-field">
                  <label class="em-field-label">{{
                    getMessage('sidepanelMarkerSelectorTypeLabel')
                  }}</label>
                ...
                <div class="em-field">
                  <label class="em-field-label">{{
                    getMessage('sidepanelMarkerMatchTypeLabel')
                  }}</label>
                  <div class="em-select-wrapper">
                    <select v-model="markerForm.matchType" class="em-select">
                      <option value="prefix">{{ getMessage('markerMatchPrefix') }}</option>
                      <option value="exact">{{ getMessage('markerMatchExact') }}</option>
                      <option value="host">{{ getMessage('markerMatchHost') }}</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div class="em-form-row">
                <div class="em-field">
                  <label class="em-field-label">{{
                    getMessage('sidepanelMarkerSelectorLabel')
                  }}</label>
                  <textarea
                    v-model="markerForm.selector"
                    class="em-textarea"
                    :placeholder="getMessage('sidepanelMarkerSelectorPlaceholder')"
                    rows="3"
                    required
                  ></textarea>
                </div>
              ...
              <div class="em-modal-actions">
                ...
                <button type="submit" class="em-btn em-btn-primary">
                  {{
                    editingMarkerId
                      ? getMessage('sidepanelMarkerUpdateButton')
          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                {{
                  getMessage('sidepanelMarkerStatsFiltered', [
                    filteredMarkers.length.toString(),
                    markers.length.toString(),
                    groupedMarkers.length.toString(),
                  ])
                }}
              </template>
              <template v-else>
                {{
                  getMessage('sidepanelMarkerStatsTotal', [
                    markers.length.toString(),
                    groupedMarkers.length.toString(),
                  ])
                }}
            ...
            <div class="em-domain-header" @click="toggleDomain(domainGroup.domain)">
              <div class="em-domain-info">
                ...
                <span class="em-domain-count">{{
                  getMessage('sidepanelMarkerDomainCount', [domainGroup.count.toString()])
                }}</span>
              </div>
            </div>
                      ...
                      <div class="em-marker-row-top">
                        ...
                        <div class="em-marker-actions">
                          <button
                            ...
                            :title="getMessage('markerValidateButton')"
                          >
                          ...
                          <button
                            ...
                            :title="getMessage('markerEditButton')"
                          >
                            <svg viewBox="0 0 24 24" width="14" height="14">
                              <path
                                stroke-linecap="round"
                          ...
                          <button
                            ...
                            :title="getMessage('markerDeleteButton')"
                          >
                            <svg viewBox="0 0 24 24" width="14" height="14">
                              <path
                                stroke-linecap="round"
        ...
        <div v-else-if="markers.length > 0 && filteredMarkers.length === 0" class="em-empty">
          <p>{{ getMessage('sidepanelMarkerNoMatch') }}</p>
          <button class="em-btn em-btn-ghost em-empty-btn" @click="markerSearch = ''">
            {{ getMessage('sidepanelMarkerClearSearch') }}
          </button>
        </div>

        <!-- Empty state -->
        <div v-else class="em-empty">
          <p>{{ getMessage('sidepanelMarkerEmpty') }}</p>
...
const groupedMarkers = computed(() => {
  ...
  for (const marker of filteredMarkers.value) {
    ...
    const domain = marker.host || getMessage('sidepanelMarkerLocalFile');
    const fullUrl = marker.url || getMessage('sidepanelMarkerUnknownUrl');
...
function createTrigger() {
  ...
  alert(getMessage('sidepanelTriggerCreateNotImplemented'));
}

function editTrigger(_id: string) {
  // V3 Trigger management not yet implemented
  alert(getMessage('sidepanelTriggerEditNotImplemented'));
}
...
async function remove(id: string) {
  try {
    const ok = confirm(getMessage('sidepanelWorkflowDeleteConfirm'));
    if (!ok) return;
...
async function deleteMarker(marker: ElementMarker) {
  try {
    const confirmed = confirm(getMessage('sidepanelMarkerDeleteConfirm', [marker.name]));
    if (!confirmed) return;
