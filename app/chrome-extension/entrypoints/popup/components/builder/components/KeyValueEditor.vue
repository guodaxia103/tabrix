  ...
  <div class="kve">
    <div v-for="(item, i) in rows" :key="i" class="kve-row">
      <input
        class="kve-key"
        v-model="item.k"
        :placeholder="getMessage('builderKveVarNamePlaceholder')"
      />
      <input
        class="kve-val"
        v-model="item.v"
        :placeholder="getMessage('builderKveResultPathPlaceholder')"
      />
      <button class="mini" @click="move(i, -1)" :disabled="i === 0">↑</button>
      <button class="mini" @click="move(i, 1)" :disabled="i === rows.length - 1">↓</button>
      <button class="mini danger" @click="remove(i)">{{
        getMessage('builderPropDeleteButton')
      }}</button>
    </div>
    <button class="mini" @click="add">{{ getMessage('builderKveAddMappingButton') }}</button>
  </div>
</template>
