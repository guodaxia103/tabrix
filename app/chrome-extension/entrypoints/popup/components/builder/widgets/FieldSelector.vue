  ...
  <div class="selector">
    <div class="row">
      ...
      <button
        class="btn-mini"
        type="button"
        :title="getMessage('builderFieldSelectorPickFromPageTitle')"
        @click="onPick"
      >
        {{ getMessage('builderFieldSelectorPickButton') }}
      </button>
    </div>
    <div class="help">{{ getMessage('builderFieldSelectorHelp') }}</div>
...
async function onPick() {
  try {
    ...
    if (!tabId) throw new Error(getMessage('builderFieldSelectorNoActiveTabError'));
    await ensurePickerInjected(tabId);
    const res: any = await chrome.tabs.sendMessage(tabId, { action: 'rr_picker_start' } as any);
    if (!res || !res.success) {
      if (res?.cancelled) return;
      throw new Error(res?.error || getMessage('builderFieldSelectorPickFailedError'));
    }
    ...
    if (sel) {
      ...
      err.value = getMessage('builderFieldSelectorNoValidSelectorError');
    }
  } catch (e: any) {
