...
- **Status**: **schema done, aggregator in progress** — `B-005` (schema) done in Sprint 2; `B-012` (aggregator) in progress in Sprint 3; `B-013` (MCP tools) pool.
...
2. **Aggregator** (`B-012`, in progress): walks `memory_sessions` where `status ∈ {completed, failed, aborted}` AND `aggregated_at IS NULL`; joins `memory_tasks` for intent; reads `memory_steps` for ordered step sequence; projects into `experience_action_paths`. Idempotent re-runs must not double-count.
...
- ⬜ `B-012` — Experience action-path aggregator (in progress).
...
| Sprint 3 (W19) | **active**          | `B-010 done · B-021 done · B-023 done · B-012 in progress · B-022 planned`. |
