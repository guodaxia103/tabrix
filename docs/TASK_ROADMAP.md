...
5. v2.4.0 release gate — **real-browser benchmark v2 framework + v2.4.0+ release gate landed in v2.4.0 (V24-05, 2026-04-23)**. Pair-aware NDJSON shape (every KPI scenario emits `first_touch` + `second_touch` records) feeds `app/native-server/src/benchmark/v24-benchmark.ts`; CLI `pnpm run benchmark:v24` (`scripts/benchmark-v24.mjs`) with gate-then-write semantics + `--baseline` auto-emits `docs/benchmarks/v24/v24-vs-v23-baseline-<date>.md`. New gate `scripts/lib/v24-benchmark-gate.cjs` enforces lane-integrity / K3 ≥ 0.85 / K4 ≤ 0.10 / `pairCount ≥ 3` per KPI scenario / baseline-table embed (hard) and emits `WARN:` on K5 < 1.5 / K6 < 0.80 / K7 > 0.20 / K8 > 0.40 (soft, evidence-only). `scripts/check-release-readiness.mjs` routes v2.4.0+ via the v24 gate, v2.3.x via the v23 gate (mutually exclusive). `--allow-missing-notes` cannot bypass the v24 content gate. The actual real-MCP run (`tabrix-private-tests` `acceptance:v2.4.0`) is owner-lane work — fast-lane never invokes the runner.
...
| `v1.2.0` | 2026-04-23 | v2.4.0 development packages V24-01 / V24-02 / V24-03 / V24-05 landed: Experience self-improvement loop (`experience_replay` + `experience_score_step` + composite session score), `tabrix_choose_context` v2 ranked replay-aware artifact, real-browser benchmark v2 framework with pair-aware K5..K8 + v2.4.0+ release gate (independent of the v2.3 gate). V24-04 (K8 token-cache) deferred to v2.5 conditional on real-MCP K8 < 0.40 evidence. |
