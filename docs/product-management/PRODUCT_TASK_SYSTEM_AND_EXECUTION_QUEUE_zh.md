...
- 本地：`docs/product-management/PRODUCT_TASK_BOARD_AND_UPDATE_TEMPLATE_zh.md`
- 飞书：`https://www.feishu.cn/docx/Tx7Ed9rHsocadWxmWOdckPounrd`

配套决策文档：

- 本地：`docs/product-management/PRODUCT_DECISION_LOG_zh.md`
- 飞书：`https://www.feishu.cn/docx/HddFdfv9HovpeyxFQ2zcqSMVnxf`

Codex 提示词文档：

- 本地：`docs/product-management/CODEX_EXECUTION_PROMPTS_T1_T16_zh.md`
- 飞书：`https://www.feishu.cn/docx/BwXudPMOKoyNJrxSeHCcL955nuf`

版本装箱文档：

- 本地：`docs/product-management/PRODUCT_VERSION_PACKAGING_AND_RELEASE_PLAN_zh.md`
- 飞书：`https://www.feishu.cn/docx/RWZRdkKCAoX6U1x0eYzcT6XNn6b`

依赖与风险文档：

- 本地：`docs/product-management/PRODUCT_TASK_DEPENDENCY_AND_RISK_REGISTER_zh.md`
- 飞书：`https://www.feishu.cn/docx/FIgwdmErwoOSOdxPZ3LcIf8JnHd`

证据索引文档：

- 本地：`docs/product-management/PRODUCT_EVIDENCE_INDEX_AND_ACCEPTANCE_ASSETS_zh.md`
- 飞书：`https://www.feishu.cn/docx/BRLFdsyrLodZfQxeaYyczR24nLh`

AI Onboarding 文档：

- 本地：`docs/product-management/AI_ONBOARDING_QUICKSTART_zh.md`
- 飞书：`https://www.feishu.cn/wiki/VIp7wNPwmi0vh2kWURecBR2bnSg`

当前能力面与边界文档：

- 本地：`docs/product-management/CURRENT_CAPABILITIES_AND_BOUNDARIES_zh.md`
- 飞书：`https://www.feishu.cn/wiki/FHnSw3s0CiXTsTkL5H9cw0DfnOd`

代码入口与责任地图文档：

- 本地：`docs/product-management/CODE_ENTRYPOINTS_AND_OWNERSHIP_zh.md`
...
4. `README.md`
5. `docs/product-management/CURRENT_CAPABILITIES_AND_BOUNDARIES_zh.md`
6. `docs/product-management/CODE_ENTRYPOINTS_AND_OWNERSHIP_zh.md`
...
| T2   | 助手命令恢复链路                    | v2.0.9   | in_progress |
...
- 产品约束：
  ...
  - `127.0.0.1` 不应被误表达为“局域网远端客户端”；UI 必须能区分“本机 HTTP（免 Token）”与“远程 HTTP（需 Token）”
...
- 验收标准：
  ...
  - UI / 文档 / 状态语义能明确区分：`本机 HTTP（免 Token）`、`远程 HTTP（需 Token）`、`stdio`
  - 客户端列表展示的是有效活跃客户端，不是历史会话堆积
  - 手动断开的客户端不会继续留在主列表
  - stale 会话可自动清理或一键清理
  - `127.0.0.1` 的本机 HTTP 客户端不会被误导性表达为“远程客户端”
  - 重新打开 Popup 不会默认再触发一轮连接动作
  - 多 session 情况下，主界面不会让用户误以为是多个不同客户端
  - `mcp`、`codex`、`claude-code` 等客户端名展示规则一致且可解释
  - 文档、UI 文案、状态接口语义一致
  - 包含真实助手链路验证
- 版本级别：`Blocker`
- 当前状态：`done`
...
- 当前状态：`in_progress`
- 最新执行进展：
  - 已完成恢复状态真相源首轮收口：扩展背景页会向本机服务回写 `/bridge/recovery/start` 与 `/bridge/recovery/finish`
  - 本机服务已新增 localhost-only 的恢复状态路由，`status/doctor` 可读到 `recoveryAttempts / recoveryInFlight / lastRecoveryAction`
  - 已补扩展侧恢复事件测试与服务端路由测试
...
- 风险与遗留：
  - 真实恢复场景仍未全部收口，浏览器未启动 / bridge 未就绪 / command channel 未就绪三类场景还需继续逐项验真
  - 需要继续补 `Codex -> Claude CLI -> Tabrix -> Chrome` 的真实恢复链路验证
...
- 将 `T1` 正式收口为 `done`
- 将 `T2` 正式切换为 `in_progress`
- 回写 `T1` 最终验收结果：`stdio-smoke`、带 Token 的远程 `smoke`、真实 Popup 打开前后活跃会话不增长

### v2026.04.15.16

- 删除顶层任务编号 `T1A`，任务池重新收口为正式的 `T1-T16`
- 将“Popup 打开即重连”“多 session 主文案过重”“泛化客户端名如 mcp 的展示策略”并回 `T1`
- 将 `T1` 状态统一收口为 `in_progress`
- 将推荐顺序恢复为 `T1 -> T2 -> T3 ...`

### v2026.04.15.15

- 记录 Popup 首轮体验修正结果：默认不再打开即重连、多 session 降为次级诊断、Popup 前后 `active/client` 对照保持 `0 -> 0`

### v2026.04.15.14

- 将“Popup 打开即重连”“多 session 主文案过重”“泛化客户端名如 mcp 的展示策略”正式纳入 `T1` 后续收口范围
- 当时曾临时调整执行顺序，现已在 `v2026.04.15.16` 纠偏回正式的 `T1-T16`

### v2026.04.15.13

- 将 `T1` 状态更新为 `done`
- 回写 `T1` 最终验收结果：Popup 两模式、默认远程、localhost/远端 HTTP 语义、客户端列表文案收口、`stdio` 与带 Token 的 `Streamable HTTP` 两种正式连接方式验证均已完成

### v2026.04.15.12

- 在 `T1` 中补充 `localhost HTTP` 与远程 HTTP 的鉴权与展示边界
- 明确 `localhost` 仅作为 `Streamable HTTP` 的本机实现细节存在：本机免 Token，非本机 HTTP 必须 Bearer
- 将“`127.0.0.1` 不应被误表达为远端客户端”补入 `T1` 约束与验收口径
