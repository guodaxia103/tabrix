...
| **DOM 智能理解与高强度脱水**   | 基于 DOM/可访问性树生成任务导向的轻量结构，减少视觉依赖  | page-agent 的 DOM 思路    | 降低 token 成本并提升定位命中率 |

### 4.2.1 DOM 智能理解借鉴清单（纳入 Program 1）

1. 分层脱水视图：`page -> region -> actionable node`，替代单一大文本输出。
2. 任务导向模式：为读、搜、填、点场景输出不同结构（`read_mode/form_mode/nav_mode`）。
3. 候选可执行打分：统一产出 `confidence`、`matchReason`、`fallbackChain`。
4. 稳定定位链：`ref + role/name + selector + fallback selector`，降低 ref 失效率。
5. Token 预算档位：`compact/normal/full` 三档输出，默认 `normal`，面向不同客户端自适应。
...
| **DOM 脱水压缩率** | > 60%                   | 同页面任务导向输出 token 与原始输出对比       |
| **定位一次命中率** | > 85%                   | click/fill 首次命中统计（含 ref/fallback）    |
...
3. **一键安装体验**：`npx tabrix setup`
4. **5 大客户端验证**：CoPaw / Claude Desktop / Cursor / OpenClaw / Claude Code
5. **DOM 脱水 v1（规则版）**：在 `chrome_read_page` 落地 `compact/normal/full` 输出档位与任务导向视图

### 中期（3–6 月）：差异化期

> 目标：**从「工具集」升级为「执行平台」**

6. **Program 1 核心**：Task/Session/Step 模型 + 超时重试 + 结果验证
7. **DOM 脱水 v2（评分版）**：候选动作打分、定位链与失败回退策略统一
8. **操作记忆 v1**：成功路径缓存，减少 LLM 调用
9. **站点适配器 v1**：高频网站（GitHub/Google/Amazon）预置知识
10. **API Gateway**：HTTP 层供 n8n/Dify 等工作流平台调用

### 长期（6–12 月）：壁垒期

> 目标：**建立网络效应和生态壁垒**

11. **插件市场 v1**：社区贡献站点适配器和工作流模板
12. **云浏览器选项**：可选的托管浏览器实例
13. **企业版**：团队协作、审计日志、SSO
14. **商业化**：按需付费的云服务 + 企业订阅
