...
> **⚠️ Superseded notice (2026-04-21)**
>
...
>   目的：保留当时把"已完成的 MKEP 阶段 + 产品愿景四性目标 + `<local-competitor-checkout>` 下 14 个开源项目的实证借鉴"汇成 Stage 3+ 执行计划的历史版本；当前版本的派生依据已经迁移，不再以本文作为后续 `feat/*` 分支或版本计划来源。

---

## 0. 产品愿景与北极星指标（NEW in v0.2）

### 0.1 产品定位（来自维护者私有产品资料 + 2026-04-21 用户定调）
...
**进度（2026-04-20，Sprint 2 · B-005）**：Schema 已落地（`experience_action_paths` / `experience_locator_prefs` 及索引见 `app/native-server/src/memory/db/schema.ts` 中的 `EXPERIENCE_CREATE_TABLES_SQL`；表名常量从 `app/native-server/src/memory/experience/index.ts` 暴露）。**仍无聚合器 / 仍无 MCP 工具**，这两者对应 Sprint 3+ 的 B-012 / B-013。
