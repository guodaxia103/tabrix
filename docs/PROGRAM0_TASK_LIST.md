...
| E10  | **tabId 被忽略**                    | bug           | #275       | `[~]` 另含 **`chrome_bookmark_add`（无 url 时）**、**`chrome_get_interactive_elements`**、**`chrome_userscript`（create / send_command）** 已支持 `tabId`/`windowId`。`network-capture` 等见扫表（#275） |
...
- `[~]` **E10（部分）**：dialog、network*request、performance*\*、bookmark_add（无 url）、get_interactive_elements、userscript create/send_command 已尊重 `tabId`/`windowId`；network-capture 等待扫表（#275）。
