...
- `intent_signature` 归一化是隐藏的难点："列出 repo X 的 issues"和"显示 repo X 的 issues"必须 hash 到同一个桶。起步：小写 + 去停用词 + 词干化；桶泄漏再调。
- `step_sequence` JSON 是 `{ toolName, argTemplate, status, historyRef }` —— 保留 `historyRef`，让 replay 能重新拉同一个 page snapshot。
