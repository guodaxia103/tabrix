...
5. v2.4.0 release gate —— **真实浏览器 benchmark v2 框架 + v2.4.0+ release gate 已在 v2.4.0 (V24-05, 2026-04-23) 落地**。Pair-aware NDJSON（每个 KPI 场景必出 `first_touch` + `second_touch` 两条记录）馈入 `app/native-server/src/benchmark/v24-benchmark.ts`；CLI `pnpm run benchmark:v24` (`scripts/benchmark-v24.mjs`) 采用 gate-then-write 语义，`--baseline` 自动产出 `docs/benchmarks/v24/v24-vs-v23-baseline-<date>.md`。新 gate `scripts/lib/v24-benchmark-gate.cjs` 强约束 lane-integrity / K3 ≥ 0.85 / K4 ≤ 0.10 / 每个 KPI 场景 `pairCount ≥ 3` / 基线对照表必须嵌入（hard），并对 K5 < 1.5 / K6 < 0.80 / K7 > 0.20 / K8 > 0.40 仅发 `WARN:`（soft，仅作证据）。`scripts/check-release-readiness.mjs` 按版本路由：v2.4.0+ 走 v24 gate，v2.3.x 走 v23 gate（互斥）。`--allow-missing-notes` 不能跳过 v24 内容门。真实 MCP 跑测（`tabrix-private-tests` 的 `acceptance:v2.4.0`）属 owner-lane —— fast-lane 不会调用。
...
| `v1.2.0` | 2026-04-23 | v2.4.0 开发包 V24-01 / V24-02 / V24-03 / V24-05 全部落地：Experience 自学习闭环（`experience_replay` + `experience_score_step` + 复合会话评分）、`tabrix_choose_context` v2 ranked replay-aware artifact、真实浏览器 benchmark v2 框架（pair-aware K5..K8）+ v2.4.0+ release gate（与 v2.3 gate 互斥按版本路由）。V24-04（K8 token cache）按 §6.4 conditional 推迟到 v2.5，仅在真实 MCP 测出 K8 < 0.40 时启动。 |
