...
**说明**：扩展已通过 Native Messaging 连接到本地 `tabrix`，HTTP/MCP 服务在指定端口正常监听。
...
**说明**：Chrome 扩展能通过 Native Messaging 与本地 `tabrix` 通信，但 HTTP 服务未成功启动。
...
| Native host 进程异常退出 | 终端运行 `tabrix doctor` 查看详细诊断       |
| 首次安装后未完成注册     | 运行 `tabrix register` 或 `tabrix setup`    |

**快速排障流程**：

```
1. 点击 Popup 的「刷新」按钮
2. 如果仍然黄色 → 打开终端运行：tabrix doctor
...
**说明**：Chrome 扩展无法与本地 `tabrix` 建立 Native Messaging 通信。
...
| Windows 注册表路径不正确      | `tabrix doctor` 会检查并给出修复建议                      |
| 扩展 ID 变更（重新加载后）    | 重新运行 `tabrix register`，因为 manifest 中记录了扩展 ID |
...
| 错误关键字                                                   | 可能原因                                          | 修复                                               |
| ------------------------------------------------------------ | ------------------------------------------------- | -------------------------------------------------- |
| `Specified native messaging host not found`                  | Manifest 未注册或路径错误                         | `tabrix register`                                  |
