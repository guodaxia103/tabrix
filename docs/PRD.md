...
- `experience_replay` v1 **landed in v2.4.0 (V24-01)** — bridged MCP tool re-runs a recorded action path through the existing extension dispatch pipeline; gated by capability `experience_replay` + `requiresExplicitOptIn`, GitHub-only `pageRole`, supported step kinds = `chrome_click_element` / `chrome_fill_or_select`. `experience_score_step` v1 **landed in v2.4.0 (V24-02)** — native MCP tool that records per-step `ClickObservedOutcome` write-backs to `experience_action_paths`; reuses the `experience_replay` capability; the replay engine itself also calls into the same write-back path on every step (so V24-01 sessions now compound counters automatically); session-end composite score (deterministic, per-`taskWeights` v1) is written by the aggregator on replay sessions only. SQLite write failures are **isolated** (`experience_writeback_warnings` table) so the user's replay path is never blocked. Richer ranked-candidate fallback ladders remain Stage 3b continuation (V24-03).
...
**Context Strategy Selector (v1 + v1.5 + v2 ranked landed; full Stage 3h DoD still pool)** — `tabrix_choose_context(intent, url?, pageRole?, siteId?)` is the **single biggest token-thrift lever** (B-018). v1 minimal slice landed 2026-04-22 (rule-based 3-strategy chooser, P0, native-handled); v1.5 added telemetry + outcome write-back + markdown branch (V23-04); **v2 (V24-03, v2.4.0) adds the deterministic ranked Experience artifact + replay-eligibility post-mortem fields, consuming V24-02 composite scores so the upstream LLM picks `experience_replay` over `experience_reuse` only when the row crosses the strict V24-03 thresholds.** See §9.3 for rationale.
...
| ------------------------ | ------------------------------------------------------------------------- | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `tabrix_choose_context`  | **v1 + v1.5 + v2 ranked replay-aware landed (V23-04 + V24-03 in v2.4.0)** | Knowledge + Experience | Given `(intent, url?, pageRole?, siteId?)`, returns the minimum-token strategy bundle. **v2 (V24-03)** adds the `experience_ranked` artifact (top-3 deterministic rank from Experience using V24-02's composite score), `replayEligibleBlockedBy` post-mortem (`capability_off` / `unsupported_step_kind` / `non_portable_args` / `non_github_pageRole` / `below_threshold` / `stale_locator` / `none`), and a `replayFallbackDepth` slot the chooser only ever reports `0` / `'cold'` (the actual ladder depth is maintained downstream by the replay engine). Telemetry table schema is **frozen at v2.3** in v2.4 to keep the existing release gate intact; ranked-depth statistics persist in v2.5. |
...
| `experience_score_step`  | **landed in v2.4.0 (V24-02)**                                             | Experience             | Let the upstream LLM (or the replay engine itself) feed per-step `ClickObservedOutcome` back into Memory and trigger session-end composite score write-back. `P1` + `requiresExplicitOptIn`; **reuses** the `experience_replay` capability (no new key — capture/use are bundled). Failure mode = isolation + structured warning row in `experience_writeback_warnings` (replay path is never stalled by SQLite I/O failures).                                                                                                                                                                                                                                                                          |
