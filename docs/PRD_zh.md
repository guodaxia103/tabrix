...
- **API Knowledge 完全缺失** —— 最大的空位（Stage 3g 的 `B-017`）；
...
**当前成熟度**：约 10% —— schema 已落地（`experience_action_paths` / `experience_locator_prefs`，`B-005`）。**聚合器还没开始写**；`B-012` 正在 Sprint 3 进行中。
...
**当前成熟度**：约 40% —— `TOOL_RISK_TIERS`（P0/P1/P2/P3）+ `requiresExplicitOptIn` 已落 `packages/shared/src/tools.ts`；P3 工具默认隐藏，除非 env allow-list 打开。
...
- capability opt-in 还是一个 bool，不是完整的 `TabrixCapability` 枚举（Stage 3f 的 `B-016`）；
- 没有用户级覆盖层。
...
Wave 1（近期可并行，互不阻塞）
  ...
  3g · API Knowledge（XHR/fetch 捕获）          （B-017 pool） ← K1 最大杠杆（数据侧）
