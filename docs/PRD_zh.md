...
**Context Strategy Selector（v1 + v1.5 + v2 ranked 已落地；完整 Stage 3h DoD 仍在 pool）** —— `tabrix_choose_context(intent, url?, pageRole?, siteId?)` 是**省 token 的最大单点杠杆**（`B-018`）。v1 minimal slice 2026-04-22 落地（规则版三策略 chooser，P0、native-handled）；v1.5 加上 telemetry + outcome 写回 + markdown 分支（V23-04）；**v2（V24-03，v2.4.0）新增确定性 ranked Experience artifact 与 replay-eligibility 复盘字段，消费 V24-02 的 composite score，让上游 LLM 只在样本越过 V24-03 严格阈值时才会把策略升级为 `experience_replay`，而不是 `experience_reuse`。** §9.3 讲为什么。
...
| 工具                     | 规划 Stage                                                               | 层                     | 用途                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------ | ------------------------------------------------------------------------ | ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tabrix_choose_context`  | **v1 + v1.5 + v2 ranked replay-aware 已落地（V23-04 + V24-03，v2.4.0）** | Knowledge + Experience | 给 `(intent, url?, pageRole?, siteId?)`，返回最小 token 的 strategy bundle。**v2（V24-03）** 新增 `experience_ranked` artifact（按 V24-02 的 composite score 给出确定性 top-3 排序）、`replayEligibleBlockedBy` 复盘字段（`capability_off` / `unsupported_step_kind` / `non_portable_args` / `non_github_pageRole` / `below_threshold` / `stale_locator` / `none`），以及一个 chooser 侧只会写 `0` / `'cold'` 的 `replayFallbackDepth` 槽位（真正的 ladder 深度由 replay 引擎在下游维护）。telemetry 表结构在 v2.4 **冻结在 v2.3 的形态** 以保住 release gate；ranked 深度统计延后到 v2.5 落库。 |
...
```
Wave 1（近期可并行，互不阻塞）
  3a · Knowledge UI Map + 稳定 targetRef       （B-010 done；B-011 v1 done —— UI-Map 消费切换留给 v2）
