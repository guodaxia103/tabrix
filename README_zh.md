...
公开文档已统一为英文，仅保留根目录下的 `README_zh.md`（本文档）与 `AGENTS.md` 作为中文入口。内部治理、PRD、路线图排期、审计门禁、验收资产等中文材料维护在飞书 `Tabrix` 知识库。
...
### 内部治理文档（飞书 Tabrix 知识库）

PRD v1、产品定位与技术原理、工具分层与风险分级、AI 开发规则、代码入口与责任地图、T4 验收门禁、OSV 安全门禁、发布前检查清单、v2 发布门禁标准、三方复用矩阵/工作流、Tabrix-First GitHub 排障 Runbook、浏览器桥接状态机设计、Browser Tool Settle Audit、Skills 目录、维护日志等维护在飞书 `Tabrix` 知识库。导航参见 [AGENTS.md](AGENTS.md)。
